...
describe('Life OS focus music catalog', () => {
  it('keeps the supplied focus sets in a separate catalog', () => {
    ...
    expect(FOCUS_MUSIC_FOCUS_CATALOG).toEqual([
      'kreamLiquidLab',
      'gioliAndromeda',
      'gioliDiesis',
    ]);
    expect(FOCUS_MUSIC_FOCUS_CATALOG.every((preset) => FOCUS_MUSIC_CATALOG.includes(preset))).toBe(true);
  });
